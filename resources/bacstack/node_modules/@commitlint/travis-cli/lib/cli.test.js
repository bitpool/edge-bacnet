...
var bin = function bin() {
	var $args = arguments;return new Promise(function ($return, $error) {
		var config;
		config = $args.length > 0 && $args[0] !== undefined ? $args[0] : {};
...
test('should throw when not on travis ci', function (t) {
	return new Promise(function ($return, $error) {
		...
		return Promise.resolve(t.throws(bin({ env }), /@commitlint\/travis-cli is inteded to be used on Travis CI/)).then(function ($await_3) {
			try {
				return $return();
			} catch ($boundEx) {
				return $error($boundEx);
...
test('should throw when on travis ci, but TRAVIS_COMMIT is missing', function (t) {
	return new Promise(function ($return, $error) {
		var env;
